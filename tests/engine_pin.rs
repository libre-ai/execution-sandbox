use libre_ai_harness::{engine_manifest_digest, profile_digest};
...
fn the_manifest_advertises_exactly_what_the_engine_offers() {
    let manifest: Value = serde_json::from_str(libre_ai_harness::ENGINE_MANIFEST)
    ...
    assert_eq!(
        advertised,
        libre_ai_harness::engine_capabilities(),
