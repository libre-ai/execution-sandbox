...
use libre_ai_harness::{PathAccess, PathAccessKind, evaluate_path_access, parse_profile};
...
fn profile() -> libre_ai_harness::HarnessProfile {
    let document: Value =
...
fn profile_with_writable(pattern: &str) -> libre_ai_harness::HarnessProfile {
    let mut document: Value =
        serde_json::from_str(CANONICAL_PROFILE).expect("the canonical profile fixture must parse");
    document["filesystem"]["writable"] = serde_json::json!([pattern]);
    let digest = libre_ai_harness::profile_digest(&document).expect("the mutation must digest");
...
fn a_multi_star_pattern_decides_in_bounded_time() {
    let profile = profile_with_writable("a*a*a*a*a*a*a*b");
    let subject = format!("/ws/{}", "a".repeat(64));
    let observation = access(&subject, PathAccessKind::Write);
    let root = Path::new(ROOT);
    // Schema compilation prepares the fixture; the bound measures only the decision.
    let started = std::time::Instant::now();
    let decision = evaluate_path_access(root, &observation, &profile);
    let elapsed = started.elapsed();
    let refusal = decision.expect_err("the crafted name matches no writable pattern");
    assert_eq!(refusal.code(), "harness.write_outside_writable_set");
    assert!(
...
/// The same class of defect one level up: `**` across segments used to be a
/// recursive OR without memoisation, exponential in the number of `**` the
/// pattern carries (4.9 s at eleven, measured by the round 4 security pass on
/// 0b5204f) — a hang is a refusal the caller never receives, exactly what the
/// intra-segment `*` fix closed at the character level.
#[test]
fn a_multi_double_star_pattern_decides_in_bounded_time() {
    let pattern = format!("{}b", "**/a/".repeat(12));
    let subject = format!("/ws/{}x", "a/".repeat(36));
    let profile = profile_with_writable(&pattern);
    let observation = access(&subject, PathAccessKind::Write);
    let root = Path::new(ROOT);
    // Schema compilation prepares the fixture; the bound measures only the decision.
    let started = std::time::Instant::now();
    let decision = evaluate_path_access(root, &observation, &profile);
    let elapsed = started.elapsed();
    let refusal = decision.expect_err("the crafted path matches no writable pattern");
    assert_eq!(refusal.code(), "harness.write_outside_writable_set");
    assert!(
        elapsed < std::time::Duration::from_secs(1),
        "the cross-segment glob decision must stay bounded"
    );
}
