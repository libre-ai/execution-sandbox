...
use libre_ai_harness::{HostFacts, parse_profile, profile_digest, resolve_controls};
...
const DIGEST_VECTORS: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/fixtures/contracts/fixtures/agent-orchestration-v1/digest-vectors.v1.json"
));
...
fn a_capability_the_engine_does_not_provide_is_refused_deny_on_missing() {
    ...
    let payload = vectors["vectors"]
        ...
        .iter()
        .find(|entry| entry["id"] == "harness-profile-v2")
        .expect("the harness-profile-v2 vector must exist")["unsignedPayload"]
        .clone();
...
/// `SO_PEERCRED` on this engine's anonymous socketpair names the process
/// that called `socketpair()` at both ends, never a distinct peer — there is
/// no `connect()` to capture a peer's identity over. A profile that
/// prescribes `verifyOsPeer` over this transport asks for a control the
/// engine has no mechanism for, and that gets refused, not silently ignored
/// (harness-profile.v2, ADR-0030): the schema now lets a profile author
/// write `verifyOsPeer: true`, so the runtime is the boundary that catches
/// it.
#[test]
fn a_profile_prescribing_verify_os_peer_is_refused_as_control_not_enforceable() {
    let mut document = canonical_document();
    document["workerTransport"]["verifyOsPeer"] = Value::Bool(true);
    let profile = parse_profile(&registry(), &with_digest_recomputed(document))
        .expect("the mutated profile still satisfies the locked schema");
    let refusal = resolve_controls(&profile, &full_facts())
        .expect_err("verifyOsPeer cannot be honoured over an anonymous socketpair");
    assert_eq!(refusal.code(), "harness.control_not_enforceable");
}
