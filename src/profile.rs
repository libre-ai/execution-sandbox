...
const PROFILE_SCHEMA: &str = "harness-profile.v2.schema.json";
...
pub struct HarnessProfile {
    ...
    worker_transport_verify_os_peer: Option<bool>,
...
impl From<ProfileWire> for HarnessProfile {
    fn from(wire: ProfileWire) -> Self {
        Self {
            ...
            worker_transport_verify_os_peer: wire.worker_transport.verify_os_peer,
...
impl HarnessProfile {
    ...
    /// `workerTransport.verifyOsPeer` as the profile states it: a free boolean
    /// since `harness-profile.v2` (ADR-0030 D1), absent when unstated.
    #[must_use]
    pub const fn worker_transport_verify_os_peer(&self) -> Option<bool> {
        self.worker_transport_verify_os_peer
    }
...
/// measured it. A pointer enters this list on one of two grounds, and names
/// which:
///
/// 1. a code path reads the field AND acts on it (refuses, applies, binds);
/// 2. the locked contract fixes the value by `const` AND the mechanism
///    behind it is unconditional in this engine — the field is then a fact
///    the schema validation already established, and reading it again would
///    be a branch no valid document can take. These members are listed in
///    [`CONTRACT_LOCKED_SURFACE`], and a test holds each of them to a `const`
///    in the schema: the day a successor contract frees one, the test goes
///    red and the field moves to ground 1, the way `verifyOsPeer` did when
///    v2 freed it (ADR-0030 D1; round 4 architecture verdict on 0b5204f).
///
...
/// The members of [`APPLIED_PROFILE_SURFACE`] admitted on the second ground:
/// held by the contract's `const`, without a mutable control branch, and by a
/// mechanism this engine applies unconditionally — the schema identity, the
/// run token every spawn frames and requires back, the deny-on-missing rule
/// `resolve_controls` applies to every required capability, and the four
/// bindings every attestation carries. `tests/effective_profile_core.rs`
/// holds each to a `const` in the locked schema.
pub const CONTRACT_LOCKED_SURFACE: &[&str] = &[
    "/schemaVersion",
    "/sandboxEngine/denyOnMissing",
    "/workerTransport/runBoundToken",
    "/attestation/signed",
