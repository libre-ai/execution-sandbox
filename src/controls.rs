...
pub fn resolve_controls(
    ...
    // `verifyOsPeer` is a free boolean since harness-profile.v2 (ADR-0030 D1).
    // Over the anonymous socketpair this engine opens, SO_PEERCRED names the
    // creator at both ends — there is no peer identity to verify — so a
    // profile prescribing the control asks for a mechanism this engine does
    // not have: refused, never approximated or silently dropped.
    if profile.worker_transport_verify_os_peer() == Some(true) {
