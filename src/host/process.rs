...
impl ConfinedOutcome {
    ...
    /// True only for a native zero exit with complete, bounded, run-bound
    /// output. EOF triggers immediate termination: a worker that closes its
    /// transport before exiting may be killed and returns false even if its
    /// output is complete and it would otherwise finish successfully.
    #[must_use]
...
pub fn spawn_confined(
    ...
    loop {
        ...
        match receiver.read(&mut buffer) {
            ...
            // verdict, blocking finding 1). The group is cleared NOW, while
            // the leader is still unreaped: a pgid whose leader is at worst
            // a zombie cannot be recycled, so the signal cannot reach a
            // stranger. Waiting first — the grace period of round 4 — reaped
            // the leader and freed the pgid before the kill was sent, a
            // mistargeted KILL as root (round 4 security verdict on 0b5204f,
            // major 1). EOF does not establish native exit: user-space
            // cleanup may close the transport first. That worker can be
            // killed even if it intended to exit zero. Preserve the actual
            // native status; complete bound output cannot imply success.
            Ok(0) => {
                reap(&mut child, plan);
...
/// Collect the already-signalled worker's status within the remaining bound.
///
/// Call only after cleanup has been signalled: `try_wait` reaps the leader,
/// so a subsequent process-group signal could target a recycled identity.
fn wait_within(
    child: &mut Child,
    started: Instant,
    max_duration: Duration,
...
/// Kill a process group by its identifier. Only ever called while the
/// leader is unreaped (alive or zombie): that is what keeps the pgid
/// reserved. Capturing the id earlier changes nothing — `Child::id()` is
/// cached at spawn — only the ordering against the wait does.
